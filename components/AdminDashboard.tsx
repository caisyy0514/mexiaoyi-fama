  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        if (success) {
          alert(`成功导入 ${codes.length} 条会员码`);
          refreshStats();
        }
      } catch (err) {
        console.error(err);
        alert(err instanceof Error ? err.message : "文件解析失败，请检查格式");
      } finally {
        setIsUploading(false);
        if (e.target) e.target.value = ''; // 清除文件选择状态
      }
    };
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex justify-between items-center">
        <div>
          ...
          <p className="text-[9px] text-blue-500 font-bold uppercase tracking-wider">系统版本：v3.1.0-Stable</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <section className="bg-white p-8 rounded-[2rem] shadow-sm border border-gray-100">
            ...
            <div className="space-y-4">
              <div className="space-y-1">
                <label className="text-[10px] font-black text-gray-400 uppercase ml-1">活动展示名称</label>
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-black text-gray-400 uppercase ml-1">兑换详细说明</label>
              ...
              <div className="flex items-center gap-6 p-6 bg-gray-50 rounded-[1.5rem] border border-gray-100">
                ...
                <div className="flex-grow">
                  ...
                  <p className="text-[10px] text-gray-400 mt-2">支持 JPG/PNG，自动适配移动端扫码</p>
                </div>
              </div>
            </div>
          </section>
        ...
        <div className="space-y-6">
          <section className="bg-gradient-to-br from-blue-700 to-indigo-900 p-8 rounded-[2rem] text-white shadow-xl relative overflow-hidden">
            <div className="relative z-10">
              ...
              <div className="mt-8 pt-6 border-t border-white/10 flex justify-between items-end">
                <div>
                  ...
                  <div className="text-[9px] font-bold text-blue-300 uppercase tracking-wider">已成功分发</div>
                </div>
                <button onClick={onReset} className="px-3 py-1 bg-white/10 hover:bg-white/20 rounded-lg text-[9px] font-black text-white uppercase transition-all">重置数据</button>
              </div>
            </div>
          ...
          <section className="bg-white p-8 rounded-[2rem] shadow-sm border border-gray-100">
            ...
            <label className={`w-full flex flex-col items-center justify-center p-8 border-2 border-dashed border-gray-200 rounded-2xl cursor-pointer hover:border-blue-400 hover:bg-blue-50/30 transition-all ${isUploading ? 'opacity-50' : ''}`}>
              <svg className="w-8 h-8 text-gray-300 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
              </svg>
              <span className="text-xs font-bold text-gray-600">{isUploading ? '处理中...' : '拖拽 Excel 到此处'}</span>
              <input type="file" accept=".xlsx,.xls,.csv" onChange={handleFileUpload} className="hidden" disabled={isUploading} />
            </label>
            <div className="mt-6">
              <textarea 
                className="w-full p-4 bg-gray-50 rounded-2xl text-[10px] font-mono border-none focus:ring-1 focus:ring-blue-400 h-32 mb-3 outline-none"
                placeholder="手动粘贴会员码，每行一个..."
                value={rawInput}
